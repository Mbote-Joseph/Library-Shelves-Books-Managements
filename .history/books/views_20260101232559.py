...
from books.forms import  BookForm, LibraryForm, ShelfForm, ShelfLibraryForm
...
class LibraryDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["shelf_form"] = ShelfForm()
        return 
     
    def post(self, request, *args, **kwargs):
        self.object = self.get_object()  # the library
        form = ShelfLibraryForm(request.POST)

        if form.is_valid():
            shelf = form.save(commit=False)
            shelf.library = self.object
            shelf.save()
            return redirect("library.details", pk=self.object.pk)  # reload page

        # if invalid, re-render with errors
        context = self.get_context_data()
        context["shelf_form"] = form
